...
import './D3SimpleDendrogramChart.scss';

class D3SimpleDendrogramChart extends React.Component {
    componentDidMount() {
        let chartRefStyle = document.defaultView.getComputedStyle(this.chartRef)
        const containerWidth = parseInt(chartRefStyle.width);
        const data = this.props.data;
        const margin = { top: 80, right: 250, bottom: 80, left: 200 };
        const width = containerWidth - margin.left - margin.right;
        const height = parseInt(chartRefStyle.height) - margin.top - margin.bottom;  
        console.log(chartRefStyle.height)
        ...
        let root = d3.tree() // 定义树
            .size([height, width])
            .separation(function (a, b) {
                return a.parent === b.parent ? 1 : 2;
            })(d3.hierarchy(data));

        let linkLine = d3.linkHorizontal()// 连接线创建器 
        ...
        let node = g.selectAll(".node") // 定位并创建到每个节点
        .data(root.descendants())
        .enter().append("g")
          .attr("class", function(d) { return "node" + (d.children ? " node--internal" : " node--leaf"); })
          .attr("transform", function(d) { return "translate(" + d.y + "," + d.x + ")"; });

        node.append("circle") // 画小圆
          .style('fill',function(d){ return z(d.depth); })
          .attr("r", 2.5);

        node.append("text") // 输出文字
            .attr("dy", 3)
    ...
    render() {
        return (
            <div className="Dendrogram-chart--simple">
                <svg ref={(r) => this.chartRef = r} width='1000px' height='2500px'></svg>
            </div>
        )
    }
}
